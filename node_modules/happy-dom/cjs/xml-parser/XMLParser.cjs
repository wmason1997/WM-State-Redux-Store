...
const HTMLElementVoid_js_1 = __importDefault(require("../config/HTMLElementVoid.cjs"));
const HTMLElementUnnestable_js_1 = __importDefault(require("../config/HTMLElementUnnestable.cjs"));
const NamespaceURI_js_1 = __importDefault(require("../config/NamespaceURI.cjs"));
const HTMLElementPlainText_js_1 = __importDefault(require("../config/HTMLElementPlainText.cjs"));
                        ...
                        if (match[1]) {
                            ...
                            if (unnestableTagNameIndex !== -1) {
                                ...
                                while (currentNode !== root) {
                                    if (currentNode[PropertySymbol.tagName].toUpperCase() === tagName) {
                                        stack.pop();
                            ...
                            if (match[2].toUpperCase() ===
                                ...
                                // Some elements are not allowed to be nested (e.g. "<a><a></a></a>" is not allowed.).
                                // Therefore we need to auto-close the tag, so that it become valid (e.g. "<a></a><a></a>").
                                const unnestableTagNameIndex = unnestableTagNames.indexOf(currentNode[PropertySymbol.tagName].toUpperCase());
                                if (unnestableTagNameIndex !== -1) {
                                    unnestableTagNames.splice(unnestableTagNameIndex, 1);
                                }
                                stack.pop();
                    ...
                    case MarkupReadStateEnum.insideStartTag:
                        ...
                        if (match[7] || match[8]) {
                            ...
                            if (!!attributeString) {
                                ...
                                while ((attributeMatch = attributeRegexp.exec(attributeString))) {
                                    if ((attributeMatch[1] && attributeMatch[2]) ||
                                        ...
                                        const namespaceURI = currentNode[PropertySymbol.namespaceURI] === NamespaceURI_js_1.default.svg &&
                                            name === 'xmlns'
                                            ? NamespaceURI_js_1.default.xmlns
                                            : null;
                            ...
                            if (hasAttributeStringEnded) {
                                ...
                                if (HTMLElementVoid_js_1.default[currentNode[PropertySymbol.tagName]] ||
                                    (match[7] &&
                                    ...
                                    plainTextTagName = HTMLElementPlainText_js_1.default[currentNode[PropertySymbol.tagName]]
                                        ? currentNode[PropertySymbol.tagName]
                                        : null;
                                    readState = !!plainTextTagName
                                        ? MarkupReadStateEnum.plainTextContent
                                        : MarkupReadStateEnum.startOrEndTag;
                                    if (HTMLElementUnnestable_js_1.default[currentNode[PropertySymbol.tagName]]) {
                                        unnestableTagNames.push(currentNode[PropertySymbol.tagName]);
                                    }
                                }
                                startTagIndex = markupRegexp.lastIndex;
                            }
